...
// summary container - list of items
    ...
    itemContainer : (childID) => cy.get(`.cart_list > :nth-child(${childID})`),
    itemQuantity : (childID) => cy.get(`:nth-child(${childID}) > .summary_quantity`),
    itemName : (childID) => cy.get(`#item_${childID}_title_link > .inventory_item_name`),
    itemDesc : (childID) => cy.get(`:nth-child(${childID}) > .cart_item_label > .inventory_item_desc`),
    itemPrice : (childID) => cy.get(`:nth-child(${childID}) > .cart_item_label > .inventory_item_price`), 
